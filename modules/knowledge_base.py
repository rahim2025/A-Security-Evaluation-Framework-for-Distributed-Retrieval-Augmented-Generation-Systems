...
class KnowledgeBase:
    ...
    def __init__(self, text_embedding_model: SentenceTransformer):
        ...
        self.noise_scale = 0.0  # Laplace noise scale; set by PrivacyPreservingRetrieval defense
    ...
    def semantic_search(self, query: str, top_k: int = 1) -> List[Tuple[Datapoint, float]]:
        ...
        # Apply Laplace noise when PrivacyPreservingRetrieval defense is active.
        # This degrades the membership signal an adversary can extract from similarity
        # scores while preserving retrieval utility at small noise scales.
        if self.noise_scale > 0.0:
            noise = torch.distributions.Laplace(
                torch.zeros_like(similarities),
                torch.full_like(similarities, self.noise_scale),
            ).sample()
            similarities = torch.clamp(similarities + noise, 0.0, 1.0)
