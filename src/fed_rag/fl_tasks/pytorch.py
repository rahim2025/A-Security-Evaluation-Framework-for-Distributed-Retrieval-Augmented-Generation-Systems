...
class BaseFLTaskBundle(BaseModel):
    ...
    attack: BaseTrainingAttack | None = None
...
class PyTorchFlowerClient(NumPyClient):
    ...
    def fit(
        ...
        if self.task_bundle.attack:
            parameters = self.task_bundle.attack.on_weights_ready(parameters)
        self.set_weights(parameters)

        trainloader = self.trainloader
        if self.task_bundle.attack:
            modified_dataset = self.task_bundle.attack.on_before_local_training(
                self.net, trainloader.dataset
            )
            if modified_dataset is not trainloader.dataset:
                trainloader = DataLoader(
                    modified_dataset,
        ...
        if self.task_bundle.attack:
            self.task_bundle.attack.on_after_local_training(self.net)

        weights = self.get_weights()
        if self.task_bundle.attack:
            weights = self.task_bundle.attack.on_weights_ready(weights)
        return weights, len(trainloader.dataset), {"loss": result.loss}
