...
**Scope of this report:** `attack/Mia_attack/` (attack implementation) and `defense/mia_defense/` (defense implementation), evaluated live against the running system (`drag-llm-service`, `drag-data-source-0/20/100`, Docker containers). `attack/Mia_attack/` is now the **only** MIA implementation in this repository — see the Revision 7.1 note below.

**Revision 7.1 (repo hygiene, not an attack/algorithm change): the stale `attack/mia/` duplicate has been removed.** A second, unmaintained MIA folder (`attack/mia/`, last touched 2026-07-02, explicitly docstring-labeled "Modelled after: DRAG baseline `modules/attacks/membership_inference.py`") existed alongside this report's actual subject, `attack/Mia_attack/`. Nothing else in the codebase imported it — `defense/mia_defense/`, `data/build_pubmedqa_corpus.py`, and every other consumer already imported exclusively from `attack.Mia_attack`. On inspection it also had a genuine, unfixed correctness bug this report's own subject fixed early on (§12.1–12.2): its "members" were sampled from `sources_0.jsonl` (which, per `reports/kb.md`, now holds PubMedQA medical-abstract content), while its "non-members" fell back to `sources_20.jsonl`/`sources_100.jsonl` (verified by direct inspection to be SQuAD/Wikipedia trivia — e.g. Notre Dame campus passages) or, if those were absent, fabricated boilerplate sentences about quantum chromodynamics. That is precisely the cross-domain non-member contamination this project's Revision 1 discovered the hard way and moved off of when it switched to same-distribution PubMedQA held-out rows (module docstring, "Why PubMedQA instead of SQuAD"). Since the duplicate was unused, unmaintained, and would have produced a misleadingly inflated AUC-ROC had anyone run it, it was deleted (`git rm -r attack/mia/`) rather than fixed in place. This closes recommendation #9 in `reports/Security_Analysis_Report.md`. No change to `attack/Mia_attack/`'s code, weights, or measured results resulted from this cleanup.

**Revision note:** this report now covers **eight successive revisions**. Revisions 1–6 are summarized in the table below (full detail retained in §2, §12). **Revision 7 is the first attempt in this project's history to actually re-tune the composite weights under a genuine train/test split, and it worked**, directly triggered by a plain question: seed 7 (unseen data) scored 0.4224 in Revision 5/6 — is that fixed? The honest answer at the end of Revision 6 was no. Revision 7 did the actual fix rather than diagnosing the problem a third time: a grid search over 650 real per-document signal rows across all 13 seeds this project has ever observed found that the composite generalizes best as **pure `decision_match`** (weight 1.0, `similarity`/`certainty`/`length_ratio` all zeroed) — then, evaluated exactly once against 5 brand-new seeds never queried before that evaluation, this produced a mean AUC of **0.620** with a 95% CI of **[0.546, 0.695]**, which excludes chance and is *higher* than the dev-set score, indicating no overfitting to the search. This is the first revision where the production composite's generalization to genuinely unseen data is measured and confirmed, not assumed or found wanting. See §2.9 for full methodology.

**Revision 8 (this revision) closes out four of Revision 6/7's own open recommendations with direct evidence, and reframes the single most-cited open question in this report's history.** Most importantly: **§12.13's "run-to-run non-determinism" — this report's own top blocking item since Revision 6 — was root-caused, and the evidence points away from inherent LLM decoding randomness and toward this session's independently-diagnosed Docker deployment fragility** (a stale bind-mounted dataset and an outdated model-server build, the same class of bug found and fixed elsewhere in this session for the DDoS and KB extraction modules). Against the now-fixed, verified-stable service, repeated identical queries and independent process restarts produced byte-identical output. A direct consequence: the consistency-score signal (§2.8), this report's own "most promising lead," was re-tested against the fixed service and returned **exactly chance (AUC 0.50) at all 3 fresh seeds tested, mechanically** — under deterministic decoding, repeating an identical question always returns the same answer, so the signal's entire premise (members and non-members differ in answer *consistency*) cannot hold on this deployment as currently configured. This reframes a promising-but-unreplicated lead into a clean negative result. Separately, three smaller open items were closed with direct measurement: `CERTAINTY_WEIGHT=0` was validated directly on the production composite (not just the diagnostic ablation); the length-normalization floor was confirmed still necessary, but for a different reason than its docstring claimed; and `rerank_with_reliability`'s interaction with MIA's results was confirmed, by reading live on-chain state and the reranker's own code, to be exactly zero. See §2.10 for full methodology.
...
| **8 (current)** | PubMedQA | Unchanged from Revision 7 (`decision_match*1.0`) — this revision validates, not re-tunes | `CERTAINTY_WEIGHT=0` ablation: 3 fresh seeds (4001–4003). Consistency-score re-test: 3 fresh seeds (6001–6003) | `CERTAINTY_WEIGHT=0` (production) mean 0.6133 vs. `=0.2` mean 0.6072 — zeroing confirmed correct. Consistency-score: **exactly 0.50 at all 3 seeds** — mechanical chance under deterministic decoding | **Root-caused, not just diagnosed:** §12.13's non-determinism finding is now attributed to Docker deployment fragility (independently fixed this session), not inherent LLM randomness. The consistency-score signal (this report's own "most promising lead") is **reframed as non-viable under this deployment's current deterministic decoding config** — a negative result, not an unresolved promising one. |
...
8. **Revision 8 — the report's own top blocking item is root-caused, and its most-cited "promising lead" is reframed as a negative result:**
   - **§12.13's run-to-run non-determinism, this report's top blocking recommendation since Revision 6, was investigated directly rather than left as a disclosed limitation.** Against the now-fixed, verified-stable `llm-service` (the same class of Docker deployment bug — a stale bind-mounted dataset, an outdated model-server build — independently found and fixed elsewhere in this session for the DDoS and KB extraction modules): 6 sequential identical queries returned byte-identical output, and 3 independent full container restarts (fresh engine init each time) returned identical output across all 3. The evidence points to this report's earlier non-determinism finding being a symptom of the same Docker fragility, not inherent LLM decoding randomness, for this deployment's actual generation config.
   - **A direct consequence: the consistency-score signal (§2.8) — described in Revisions 6 and 7 as "the most promising and least replicated lead" — was re-tested against the fixed service at 3 fresh seeds and returned exactly AUC 0.50 in all three, mechanically.** Under deterministic (temperature=0.0) decoding, repeating an identical question always returns the same answer for both members and non-members, so consistency=1.0 for everyone is a certainty of greedy decoding, not a measurement of anything. The original 0.49–0.75 spread is now attributed to the same environment instability just root-caused, not a real membership signal. **This is a clean negative result, not an unresolved promising one.**
   - `CERTAINTY_WEIGHT=0` was validated directly on the **production** composite (previously only tested on the separate diagnostic ablation): 3 fresh seeds, production weights (certainty=0) mean AUC 0.6133 vs. certainty restored to 0.2 mean AUC 0.6072 — confirms zeroing was the right call, now on the actual deployed formula.
   - The length-normalization floor (100 chars) was re-validated against 50 fresh live responses from the fixed service: every response was a bare 2–3 character "Yes"/"No". The floor is still measurably necessary, but for a different reason than its docstring states — not compensating for a leaked-token bug (fixed), but because PubMedQA yes/no/maybe answers under this system's prompt are genuinely that short.
   - `rerank_with_reliability`'s interaction with MIA's results — flagged as an untested confound in Revision 6 — was settled by reading live on-chain scores directly: `source_0`, `source_20`, `source_100` are all at the initialized baseline (10000.0), and `reranker.py`'s own min-max normalization returns exactly zero for a constant input array. `rerank_with_reliability` has had **zero effect** on any MIA result reported in any revision.

**Security impact.** For the first time in seven revisions, the production attack's generalization to genuinely unseen data is **measured and confirmed**, not assumed, hoped for, or found wanting: mean AUC 0.620 on 5 fresh seeds, statistically excluding chance. This is a `LOW`-to-`MEDIUM` tier result by this report's own thresholds (§1.4) — a real, modest, confirmed privacy signal, not a severe vulnerability. The result is specific to this system, model, and dataset, not a universal claim about MIA composites in general (§2.9). **Revision 8 adds confidence rather than changing this headline number**: the infrastructure source of this report's largest disclosed uncertainty has been identified and addressed, and every ablation tested this revision confirmed rather than overturned Revision 7's production configuration — though a full re-run of the primary composite's own seed sweep against the now-fixed service, to see whether the originally-reported AUC swings shrink, has not been performed (§15).
...
## 2.10 Revision 8 — root-causing the non-determinism finding, and validating three open recommendations directly

**Why this section exists.** §12.13 (Revision 6) disclosed run-to-run non-determinism as this report's single largest limitation and top blocking recommendation, but explicitly left root-causing it out of scope ("would itself require investigating a different component under a different level of authorization"). §14 also carried three smaller, unactioned recommendations forward across two revisions: test `CERTAINTY_WEIGHT=0` on the production composite (not just the diagnostic), re-verify the length floor, and check the `rerank_with_reliability` interaction. This revision does all four, with direct measurement rather than further disclosure.

**Root-causing §12.13's non-determinism.** This session independently diagnosed and fixed two Docker deployment bugs affecting the shared `llm-service` (documented in this project's DDoS and KB extraction fix logs): the container was, at different points, running a stale bind-mounted dataset from a different checkout, and a build of `open_model.py` older than this report's own partial role-token fix — producing exactly the symptom §12.13 cited as its evidence for non-determinism ("leaked chat-template role tokens and wildly variable response lengths"). Against the now-fixed, verified-stable service, using the exact `/query` endpoint and `temperature=0.0` configuration `mia_attack.py` itself uses:
- 6 sequential identical calls in one session: byte-identical output every time.
- 3 independent full process restarts (fresh engine init, fresh compilation each time — the closest reproducible proxy to "separate live invocations"), same 3 test questions each time: identical output across all 3 restarts.
- The service runs single-threaded, so concurrent-request batching effects (a separate, real phenomenon under some serving stacks) don't enter into this deployment's request path either.

**Conclusion (evidence-based): for this deployment's actual generation config, decoding is deterministic session-to-session, at least for short yes/no/maybe-style answers.** The non-determinism §12.13 observed is more plausibly explained by this project's demonstrated Docker-deployment fragility — the same failure mode independently found and fixed twice elsewhere this session — than by inherent LLM sampling randomness. This reframes, but does not fully close, §12.13: a full re-run of the primary composite's own multi-seed sweep against the now-fixed service, to directly check whether the originally-reported swings (0.6432/0.5440/0.3968) shrink or vanish, was not performed this revision (see §15).

**Consequence for the consistency-score signal (§2.8).** If the original AUC spread (0.75/0.49/0.57 across seeds 0/1/42) reflected environment instability rather than a real effect, re-testing under the fixed service should collapse it. It did, completely: 3 fresh seeds (6001/6002/6003, `run_consistency_pilot.py`, n_probes=5) all returned `mean_member_consistency = mean_non_member_consistency = 1.00`, AUC exactly **0.50** in every seed. This is mechanical, not a close call: at `temperature=0.0`, repeating an identical question 5 times returns the identical answer every time for both members and non-members — consistency=1.0 for everyone is a certainty of greedy decoding, carrying no information about membership. **The signal is structurally non-viable on this deployment as currently configured** — it would need genuine sampling stochasticity (`temperature > 0`) to have any chance of working, which would reopen the exact "real signal or environment noise" question this section just closed. §2.8, §12.12, §14, and §17's characterization of this as "the most promising lead" is superseded by this finding.

**`CERTAINTY_WEIGHT=0` validated on the production formula, not just the diagnostic.** §14 had carried this as an open recommendation since Revision 6 — the ablation composite (§2.6) tested certainty at 0 alongside other reweighted terms, but never in isolation on the actual deployed formula. `attack/Mia_attack/certainty_weight_ablation.py` calls `MIAAttack._probe_documents()` once per seed (the expensive, real-LLM step) and recomputes AUC from the same per-document signals at two weight settings, so both numbers come from identical probes with no resampling confound:

| Seed | AUC, production (`CERTAINTY_WEIGHT=0`) | AUC, certainty restored (`=0.2`) |
|---|---|---|
| 4001 | 0.7000 | 0.6920 |
| 4002 | 0.5400 | 0.5224 |
| 4003 | 0.6000 | 0.6072 |
...
Adding certainty back (delta −0.0061) does not help — confirms Revision 7's zeroing decision on the actual production composite, at 3 fresh seeds never used anywhere else in this project's history.

**Length-normalization floor (100 chars) re-validated, for a different reason than its docstring states.** `defense/mia_defense/validate_length_floor.py` collected 50 fresh live responses (25 members + 25 non-members, seed 5001) from the fixed service: every single response was a bare "Yes"/"No" (2–3 characters). `calibrate_target_length(floor=100)` → 100; without the floor → 3. The floor remains necessary, but not because of the leaked-token bug its docstring attributes it to (that bug is fixed) — PubMedQA yes/no/maybe answers under this system's own terse system prompt are genuinely, inherently 2–3 characters long, and any unfloored calibration would collapse regardless of the role-token bug's status. (Docstring update not made this revision — a documentation-only change, not a behavior change.)

**`rerank_with_reliability` interaction — settled directly from code and live state, not inferred.** `drag_llm_service/configs/config.yaml` has this enabled by default, and MIA's signal comes 100% from `/query` responses that pass through it — a real, previously-unchecked confound candidate for §12.13's swings. Reading live on-chain scores (`DragScoresClient.get_scores_batch`) found `source_0`, `source_20`, `source_100` all at `10000.0`, the unchanged initialized baseline. `reranker.py`'s `_minmax()` on a constant array returns all-zeros by construction, so `reliability_norm` contributes exactly 0 to every candidate's rerank score regardless of `reliability_weight`. This is a code-confirmed fact: `rerank_with_reliability` has had zero effect on any AUC number in this report's eight-revision history, since none of MIA's own `/query` traffic ever writes a score update (only `/query_analyze` does, and its only caller anywhere in this repo is a standalone manual smoke test).

**What Revision 8 does not do.** It does not re-run the primary composite's full multi-seed sweep against the fixed service (a larger undertaking than these four targeted checks — see §15/§16), and the determinism tests above use short, single-word/yes-no-maybe-style answers specifically — MIA's paraphrase-probe prompts weren't separately stress-tested for longer, more open-ended generations where more token-level decoding surface exists even under greedy decoding.
...
Composite formula and production weights (`DECISION_WEIGHT=0.55, SIM_WEIGHT=0.20, CERTAINTY_WEIGHT=0.10, LEN_WEIGHT=0.15`) were **unchanged since Revision 4 through Revision 6** — Revision 6 only added diagnostics that tested whether they should change, per §14's recommendation to gate any change on a properly pre-registered evaluation rather than the same data used to motivate it. **Revision 7 (§2.9) acted on that recommendation and changed them**: production weights are now `DECISION_WEIGHT=1.00, SIM_WEIGHT=0.00, CERTAINTY_WEIGHT=0.00, LEN_WEIGHT=0.00`, chosen by a grid search on 13 dev seeds and confirmed on 5 fresh held-out seeds (mean AUC 0.620, 95% CI excludes chance).
...
Consistency-score pilot, 3 seeds (10+10 docs each, 5 probes/doc) — Revision 6, as originally measured
Seed 0    75.0%  🟢 Strong
Seed 42   57.0%  🟡 Modest
Seed 1    49.0%  🔴 No signal
Mean: 60.0%, spread 49-75%   ⚠️ Most promising single lead this revision, also the least replicated

Consistency-score pilot, 3 FRESH seeds (6001/6002/6003) — Revision 8, re-tested against the now-fixed service (§2.10)
Seed 6001  50.0%  🔴 Exactly chance
Seed 6002  50.0%  🔴 Exactly chance
Seed 6003  50.0%  🔴 Exactly chance
Mean: 50.0%, spread 0    🔴 SUPERSEDES the row above — mechanical chance under deterministic decoding, not a promising lead
...
**Scope of the fix taken this session (Revision 6).** None — root-causing and fixing `drag_llm_service`'s apparent decoding non-determinism and/or chat-template leakage is out of scope for this report's attack/defense code, and would itself require investigating a different component under a different level of authorization (similar in spirit to the Docker-mount finding of Revision 5, §12.9). This is disclosed as a limitation and a recommendation (§14, §16), not silently worked around.

**Update, Revision 8 (§2.10): root-caused, with direct evidence.** A later session independently diagnosed the shared `llm-service` container as running a stale bind-mounted dataset and an outdated model-server build — the same class of Docker deployment bug found and fixed twice elsewhere in this project. Against the fixed, verified-stable service, 6 sequential identical queries and 3 independent full container restarts all produced byte-identical output. The evidence now points to this section's non-determinism finding being a symptom of that infrastructure fragility, not inherent LLM decoding randomness, for this deployment's actual (temperature=0.0, short-answer) generation profile. This is evidence-based reframing, not full closure: a direct re-run of *this section's own* three-run seed-0 comparison, or of the primary composite's full multi-seed sweep, against the now-fixed service was not performed — see §15.
...
**Resolved this revision (Revision 8, §2.10) — carried here with outcome, not deleted, so the recommendation history stays auditable:**
- ~~Investigate and, if feasible, mitigate `drag_llm_service`'s run-to-run non-determinism~~ — **root-caused**, not merely mitigated: evidence points to Docker deployment fragility (independently fixed this session), not inherent decoding randomness. Downgraded from Blocking; the residual item is a full re-run of the historical seed sweep to confirm the swings shrink (moved to High priority below).
- ~~Test setting `CERTAINTY_WEIGHT` to 0 in the production composite~~ — **done**, 3 fresh seeds, confirms Revision 7's zeroing.
- ~~Re-verify the floor value against observed response lengths~~ — **done**, 50 fresh responses; floor confirmed necessary for a different reason than originally documented.
- ~~Check the `rerank_with_reliability` interaction~~ — **done**, code- and on-chain-confirmed zero effect.
- **New finding, not a prior recommendation:** the consistency-score pilot (§2.8), previously flagged as "the most promising lead," is now a **closed negative result** — re-tested at 3 fresh seeds against the fixed service, exactly chance (0.50) every time, mechanically explained by deterministic decoding. Do not pursue further replication of this specific signal unless the deployment's `temperature` is changed from 0.0.

**Blocking (must resolve before trusting any composite AUC claim)**
- **Directly re-run the primary composite's own historical multi-seed comparison (the same seeds/queries behind §12.13's 0.6432/0.5440/0.3968 swing) against the now-fixed service**, to confirm the swing shrinks or disappears rather than assuming it does from the smaller checks in §2.10. This is the one piece of direct confirmation Revision 8 did not perform.
...
- Update `calibrate_target_length()`'s docstring to reflect §2.10's finding (the floor guards against genuinely short PubMedQA answers, not the now-fixed leaked-token bug) — documentation-only, not yet done.

**Low priority**
- Revisit the sanitizer's thresholds only if a defense targeting verbatim leakage specifically becomes load-bearing.
- If the consistency-score signal is ever revisited, it would first require the deployment to run with `temperature > 0` — otherwise the mechanical chance result in §2.10 will simply reproduce.

---

# 15. Limitations

- **The live LLM service's Revision 6 non-determinism finding is now root-caused (Docker deployment fragility) rather than assumed inherent, but not fully closed** (§2.10, §12.13) — the evidence (byte-identical output across 6 calls and 3 restarts) is direct, but the original historical seed sweep that produced the 0.6432/0.5440/0.3968 swing was not itself re-run against the fixed service. Until it is, "root-caused" should be read as strong evidence, not full proof that every prior confidence interval is now trustworthy as computed.
- **The consistency-score signal is now a closed negative result on this deployment, not an open lead** — re-tested at 3 fresh seeds against the fixed service, exactly chance every time (§2.10). It would need genuine sampling stochasticity (`temperature > 0`) to have any chance of carrying signal, which is not this deployment's current configuration.
- **The primary composite's held-out generalization remains formally unresolved at n=10** — its 95% CI includes chance; only `decision_match` alone has a CI that excludes it. (This is a separate question from the composite Revision 7 actually put into production, which is `decision_match` alone and does have a passing CI — see below.)
...
- **The calibrated length-normalization floor (100 chars) is confirmed necessary by direct measurement (§2.10), but its exact value is still not derived from anything beyond a single 50-response sample** — a smaller version of the same "arbitrary parameter" issue it was built to fix, now with an accurate rather than incorrect docstring rationale pending (not yet updated).
...
- **Revision 8's determinism checks (§2.10) used short, single-word/yes-no-maybe-style answers specifically** — MIA's paraphrase-probe prompts weren't separately tested for longer, more open-ended generations, where more token-level decoding surface exists even under greedy decoding.
...
- **Directly re-run the historical seed sweep behind §12.13's swing against the now-fixed service** — the one direct confirmation Revision 8's root-cause finding is still missing.
...
- ~~Consistency-score replication at full scale~~ — **superseded**: Revision 8 replicated it at 3 fresh seeds and found a clean mechanical negative result (§2.10). Would only be worth revisiting if the deployment's decoding temperature changes from 0.0.
- ~~A `CERTAINTY_WEIGHT=0` ablation run directly on the production composite~~ — **done**, Revision 8 (§2.10).
...
**Overall effectiveness:** best described, as of Revision 8, as **unresolved for the full composite, statistically confirmed-and-production-validated for `decision_match` alone** — Revision 7's re-tuned, pure-`decision_match` composite is what's actually deployed, and it has a fresh-seed CI that excludes chance (§2.9). The defense picture improved concretely across Revisions 6 and 8 — the content-level defense is a genuine, validated addition, and Revision 8 closed out the `CERTAINTY_WEIGHT` and length-floor questions with direct measurement rather than further disclosure. **Revision 8's central contribution is epistemic, not a new number**: the non-determinism this report spent two revisions worrying about has been traced to a specific, independently-fixed infrastructure bug rather than left as an open, possibly-inherent limitation — which in turn means the "most promising lead" it was entangled with (consistency-score) is now correctly understood as a mechanical artifact of that same instability, not a real signal awaiting more data.

**Research contribution:** a worked demonstration that (a) testing a critique's proposals empirically, rather than accepting or dismissing them by argument, regularly produces a mixed verdict — some ideas work, some don't, some remain genuinely open — and that mix is more useful to a reader than a uniformly positive or uniformly skeptical write-up would be; (b) an evaluation's own infrastructure (here, the live LLM service) can be a source of measurement noise as large as anything under deliberate study, and disclosing that noise once discovered matters more than pretending measurements are cleaner than they are — **and, per Revision 8, that noise is sometimes tractable: root-causing it, rather than only disclosing it, can turn a project's most-cited unresolved limitation into a closed one and its most exciting unreplicated lead into a clear negative result in the same pass**; (c) a defense parameter "measured from real data" is only as trustworthy as that data's own stability, and needs its own sanity checks (the floor guard) rather than blind trust that measurement beats guessing — confirmed a second way in Revision 8, where the floor turned out to guard against a real phenomenon (genuinely terse gold answers) after the original phenomenon it was built for (the leaked-token bug) was fixed out from under it.

**Lessons learned:** (1) test a critique's proposals one at a time and report each outcome honestly, including the ones that don't pan out — a mixed result is more credible than a uniformly confirming one; (2) when a live system's behavior is suspected to drift, check directly (a manual query in this case) rather than assuming the code is at fault; (3) a "calibrate from data" defense parameter needs a floor or other sanity bound, because real data can itself be anomalous; (4) an ablation or ad hoc reweighting computed on the same seeds used for every other measurement is evidence toward a future change, not a license to make that change immediately; (5) a striking single-seed result (the 0.75 consistency-score AUC) is exactly the kind of finding this project has learned, across six revisions, not to trust without replication — and replication is what caught it here too, in Revision 8, definitively; (6) a limitation labeled "out of scope, would require different authorization" is worth periodically revisiting rather than treating as permanently closed — Revision 8's root-cause investigation was possible precisely because a later session had independent authorization and reason to touch the same shared infrastructure for a different module.
