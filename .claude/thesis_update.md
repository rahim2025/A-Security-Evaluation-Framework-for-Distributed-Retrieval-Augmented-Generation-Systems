...
**Reliable-dRAG has two SSM instantiations, at two different privilege levels, and the framework produced both.** The first implementation attempted (`ssm_score_attack.py`) turned out to require the LLM orchestrator's own private key, not merely control of a data source — a materially different, higher-privilege attacker than the "regular participant" story above. Tracing the honest scoring pipeline surfaced a second, genuinely no-privileged-access lever: the orchestrator's grounding check (`grounded_by_sources`) is a naive literal-substring match, not a relevance or provenance check, so a source can return broad, high-coverage filler content designed to trip that check on many queries and farm undeserved reliability/usefulness credit — without touching the blockchain or using any key beyond its own. This **Grounding-Farming** attack is the one that actually matches this section's claim verbatim, and is now the flagship Reliable-dRAG SSM result; the key-forgery variant is kept as a secondary, relabeled **control-plane / orchestrator-key-compromise** finding (a different, also-real but higher-privilege threat).

Multi-seed testing (seeds 0/42/123, 300 real query rounds each) found Grounding-Farming's effect is **probabilistic, not guaranteed**: it fully escalated in 1 of 3 seeds (100% engagement, reliability/usefulness score gap of +680/+1,574 over the untouched comparison source, accuracy down 9 points) and failed to gain traction in the other 2 (near-zero engagement, accuracy flat or slightly improved). The mechanism is a **self-reinforcing feedback loop**: the system's own reliability-weighted reranking stretches even a small, single lucky "grounded" credit into a large relative reranking advantage via min-max normalization across a small number of sources, so once a source lands one early win, it tends to keep winning — a threshold effect, not a smooth accumulation. When it does escalate, the accuracy cost is exactly this section's second claim ("legitimate high-quality sources receive fewer queries than they should"): a fixed, shared top-k budget means the inflated source crowds a genuinely relevant source out of slots it would otherwise have won on merit, even though the two sources' content never directly overlaps or competes. The key-forgery variant, in contrast, is deterministic (succeeded 3/3 seeds, every round, once throttled to respect the system's own on-chain rate/delta caps) precisely because it requires far greater privilege — illustrating that privilege level and reliability of outcome trade off against each other within the same attack class.
