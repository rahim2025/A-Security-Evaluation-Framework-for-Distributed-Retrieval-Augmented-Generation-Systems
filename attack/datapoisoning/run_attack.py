...
from datapoisoning.data_poisoning_attack import DataPoisoningAttack

# ---------------------------------------------------------------------------
# Config — env vars let Docker override these; localhost defaults work locally
# ---------------------------------------------------------------------------
LLM_SERVICE_URL = os.getenv("LLM_SERVICE_URL", "http://localhost:9000")
DATA_DIR = os.getenv(
    "DATA_DIR",
    os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'polluted_token'))
)
LOG_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'attack_logs'))

DATA_SOURCES = [
    {"name": "sources_0",   "url": os.getenv("DATA_SOURCE_0_URL",   "http://localhost:8001")},
    {"name": "sources_20",  "url": os.getenv("DATA_SOURCE_20_URL",  "http://localhost:8002")},
    {"name": "sources_100", "url": os.getenv("DATA_SOURCE_100_URL", "http://localhost:8003")},
]
...
def main():
    ...
    parser.add_argument('--max-docs', type=int,   default=50,
                        help='Max base docs per source before amplification '
                             '(keep low — server re-embeds all docs on each /poison call)')
