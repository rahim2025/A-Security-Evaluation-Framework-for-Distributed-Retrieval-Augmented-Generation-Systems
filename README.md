...
FedRAG includes experimental attack utilities for comparing centralized,
federated, and distributed RAG security behavior:

- `DataPoisoningAttack` injects corrupted training examples.
- `MembershipInferenceAttack` infers whether query-like text is likely present
  in a knowledge store using retrieval confidence.
- `KnowledgeExtractionAttack` probes the public retrieval interface to recover
  stored knowledge nodes.

For a DRAG-compatible comparison, run:

```sh
python run_security_evaluation.py \
  --drag-root /Users/tanzim_safin/Downloads/DRAG-main-2 \
  --dataset-source huggingface
```

The runner uses DRAG's MMLU dataset config, `llama3.2:3b` model metadata, and
the same `sentence-transformers/all-MiniLM-L6-v2` retrieval embedding model. It
writes `logs/security_evaluation/EVALUATION_MATRIX.md`,
`logs/security_evaluation/evaluation_matrix.csv`, and raw JSON results.

For a federated client-side evaluation with at least 10 clients, run:

```sh
python run_federated_security_evaluation.py --num-clients 10
```

This runner splits data IID across clients, poisons selected malicious clients,
runs client-local membership inference and knowledge extraction, then applies
client-side defenses: poisoning inspection/quarantine, score masking, query rate
limiting, and extraction anomaly detection. It writes a Markdown matrix, CSV
matrix, and raw JSON under `logs/federated_security_evaluation/`.
