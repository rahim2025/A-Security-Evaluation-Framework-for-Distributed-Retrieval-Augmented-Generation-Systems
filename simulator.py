...
from modules.defenses import PrivacyPreservingRetrieval
...
def run_simulation(cfg: Namespace):
    ...
    # === PRIVACY-PRESERVING RETRIEVAL DEFENSE ===
    _ppr_defense = None
    _ppr_cfg = getattr(getattr(cfg, 'defense', None), 'privacy_preserving_retrieval', None)
    if (
        _ppr_cfg is not None
        and getattr(cfg.defense, 'enabled', False)
        and getattr(_ppr_cfg, 'enabled', False)
        and cfg.rag.network_type == "DRAG"
    ):
        _ppr_defense = PrivacyPreservingRetrieval(
            noise_scale=getattr(_ppr_cfg, 'noise_scale', 0.1),
            noise_mechanism=getattr(_ppr_cfg, 'noise_mechanism', 'laplace'),
        )
        _ppr_defense.apply(rag_net)
        logger.info("=" * 50)
        logger.info("PRIVACY-PRESERVING RETRIEVAL DEFENSE ACTIVE")
        logger.info(f"  noise_mechanism : {_ppr_defense.noise_mechanism}")
        logger.info(f"  noise_scale     : {_ppr_defense.noise_scale}")
        logger.info("=" * 50)
    ...
    # Log Privacy-Preserving Retrieval defense stats if active
    if _ppr_defense is not None:
        ppr_stats = _ppr_defense.get_stats()
        logger.info("=" * 50)
        logger.info("PRIVACY-PRESERVING RETRIEVAL DEFENSE STATS")
        logger.info(f"  Mechanism      : {ppr_stats['noise_mechanism']}")
        logger.info(f"  Noise Scale    : {ppr_stats['noise_scale']}")
        logger.info(f"  Peers Protected: {ppr_stats['peers_protected']}")
        logger.info("=" * 50)
        eval_results['ppr_defense_stats'] = ppr_stats
        ppr_logger = exp_logger.get_yaml_logger("ppr_defense_stats")
        ppr_logger.log(ppr_stats)
        ppr_logger.save()
