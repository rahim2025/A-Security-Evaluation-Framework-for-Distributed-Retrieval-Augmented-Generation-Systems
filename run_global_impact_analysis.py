...
This script shows exactly how client-level attacks (Data Poisoning,
KB Extraction, Node Availability) degrade the ENTIRE federated RAG
system's LLM performance — and how much each defense recovers.

No GPU · No Hugging Face downloads · No Ollama server required.
Uses a deterministic hash-based simulator for full reproducibility.

────────────────────────────────────────────────────────────────────
USAGE
────────────────────────────────────────────────────────────────────

  # Full analysis — all attacks, all intensities (light / medium / heavy)
  python run_global_impact_analysis.py

  # Fewer clients/examples for a quick smoke-test
  python run_global_impact_analysis.py --num-clients 6 --num-examples 60

  # Only data poisoning and node removal
  python run_global_impact_analysis.py \\
      --attacks data_poisoning node_availability \\
      --node-attack-types node_removal byzantine
...
def _print_global_per_client_table(report: "GlobalImpactReport") -> None:  # type: ignore[name-defined]
    ...
    for scenario in dp_scenarios:
        ...
        print()
        print(f"  ┌─ Intensity: {lvl}  ({n_aff}/{n_tot} clients poisoned) " + "─" * (W - 40) + "┐")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="FedRAG — Global System Impact Analysis",
    ...
    p.add_argument(
        ...
        help="Fraction of each malicious client's data to poison (default: 0.5). "
             "Range 0.0–1.0. Higher = more data corrupted per client.",
    )
    p.add_argument(
        "--quarantine-threshold", type=float, default=0.5,
        help="Defence sensitivity: risk score to trigger client quarantine (default: 0.5).",
    )
    p.add_argument(
        "--attacks", nargs="+",
...
def main() -> None:
    ...
    if not args.quiet:
        ...
        print("╠══════════════════════════════════════════════════════════╣")
        print(f"║  Clients       : {args.num_clients:<40}║")
        print(f"║  Examples      : {args.num_examples:<40}║")
        print(f"║  Seed          : {args.seed:<40}║")
        print(f"║  Attacks       : {', '.join(args.attacks):<40}║")
        print(f"║  Intensities   : {', '.join(args.intensities):<40}║")
        print(f"║  Poison ratio  : {args.poisoning_ratio:<40}║")
        print(f"║  Quarantine    : {args.quarantine_threshold:<40}║")
        print(f"║  Dataset       : {str(args.dataset_jsonl or 'synthetic'):<40}║")
        print(f"║  Output dir    : {args.output_dir:<40}║")
        print("╚══════════════════════════════════════════════════════════╝")
    ...
    if args.dataset_jsonl:
        ...
        if not p.exists():
            print(
                ...
                f"\n  If you don't have a JSONL file, just omit --dataset-jsonl and"
                f"\n  the script will generate a synthetic dataset automatically:\n"
                f"\n      python run_global_impact_analysis.py\n"
            )
            sys.exit(1)
        dataset_path = str(p)
    ...
    # ── filter attack types based on --attacks flag ───────────────────────────
    poison_types_to_test = ["wrong_answer"]           # always use wrong_answer for main table
    node_types = args.node_attack_types if "node_availability" in args.attacks else []

    # Conditionally disable attacks
    ...
    # Monkey-patch evaluator to skip unwanted attacks
    ...
    print(f"  {args.output_dir}/global_attack_matrix.csv  ← flat CSV for Excel/Pandas")
